...
class Product extends CI_Controller {
	...
	function view_product_per_category($alias)
	{
		...
		$this->load->library('pagination');
		$total_item=$this->product_model->count_get_active_product_list($cat_name);
		...
		$this->data['product'] = $this->product_model->get_active_product_list_paging($cat_name, $offset, $num);
		$this->data['pagination'] = $this->pagination->create_links();
	...
	function search()
	{
		...
		if($search==""){
			...
			$this->data['page_title'].='Search Results for "'.$search.'"';
			$temp = array();
			$query="select * from product_tb where active = 1 and ( `name`";
			$temp = explode(' ',$search);
			$n=0;
			if ($temp) foreach ($temp as $t)
			{
				if($n==0)$query = $query." like '%".esc($t)."%' or `category_name` like '%".esc($t)."%' or `sub_category_name` like '%".esc($t)."%'";
				else 
				$query=$query." or `name` like '%".esc($t)."%' or `category_name` like '%".esc($t)."%' or `sub_category_name` like '%".esc($t)."%' ";
				$n++;
			}
...
//		$this->data['product']=$this->product_model->get_active_product_list2($sub_name, $this->data['cat']['name']);	
		...
		$this->load->library('pagination');
		$total_item=$this->product_model->count_active_product_list2($sub_name, $this->data['cat']['name']);
		...
		$this->data['product'] = $this->product_model->get_active_product_list2_paging($sub_name, $this->data['cat']['name'], $offset, $num);
		$this->data['pagination'] = $this->pagination->create_links();
	...
	function product_detail(){
		...
		$this->data['sku']=$this->product_model->get_active_sku_list2($id);
	...
	function view_product_detail($alias='')
	{
		...
		if($cat_al)foreach($cat_al as $ca){
			$this->data['cat']=$this->category_model->get_selected_category_data3($ca);
		...
		$this->data['sku']=$this->product_model->get_active_sku_list2($id);
		$this->data['related']=$this->product_model->get_selected_related_product($id);
