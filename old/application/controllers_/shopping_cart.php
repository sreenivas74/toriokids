	...
	function add_coupon(){
		...
		if($data){
			if(!$data2){
				...
				else echo json_encode(array('status'=>0,'msg'=>'Total must be greater than '.money2($data['minimum_sub'])));exit();
...
//		if(!$_POST)
	...
	//echo $datae['voucher_id'];
		...
		if($this->session->userdata('user_logged_in')){
        	...
        	$this->data['shipping_info']=$this->session->userdata('address_guest');
			$this->data['discount']=0;
			$this->data['total_actual_weight']="";
			$this->data['disc']=$this->discount_model->get_discount_list();
