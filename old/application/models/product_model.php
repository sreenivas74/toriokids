...
class product_model extends CI_Model{
	...
	function get_product_by_id_on_flash_sale($id){
		...
		left join template_name_tb tp on tp.id=p.template_id where p.id='".esc($id)."'";
		$query = $this->db->query($q);
		return $query->row_array();
	}
	...
	function get_best_seller_paging($offset,$num)
	{
		...
		$q = "select p.*, tp.name as template_name from product_tb p
		left join template_name_tb tp on tp.id=p.template_id
		where p.`active`=1 and best_seller=1 order by `id` DESC limit ".esc($offset).",".esc($num)."";
		$query = $this->db->query($q);
		return $query->result_array();	
	}
	...
	function get_sale_paging($offset,$num)
	{
		$q = "select p.*, tp.name as template_name from product_tb p
		left join template_name_tb tp on tp.id=p.template_id
		where p.`active`=1 and sale_product=1 order by `id` DESC limit ".esc($offset).",".esc($num)."";
