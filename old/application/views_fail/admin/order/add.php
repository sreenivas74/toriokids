...
<table class="defAdminTable" width="70%">
	<tbody>
        ...
        <tr>
        	<td>Province</td>
            <td><input type="text" class="txtField" name="province" id="province" onblur="get_city(this.value)" /> <span style="color:red">*</span></td>
        </tr>
        <tr>
        	<td>City</td>
            <td><select class="txtField" name="city" id="city" onchange="get_method(this.value);">
            	<option value="0">-- select province first --</option>
                <?php /*?><?php if($city) foreach($city as $city_list){ ?>
        ...
        <tr>
        	<td>Shipping Cost</td>
            <td><select class="txtField" name="shipping_cost" id="shipping_cost">
...
if($product){
    ...
    <tr class="form_data">	
        ...
        <td valign="top" align="right"><input type="text" class="price" name="price_<?php echo $list['id'] ?>" id="price_<?php echo $list['id'] ?>" value="<?php echo $price?>"></td>
...
function form_submit(){
	...
	if(!name || !address || !province || !phone || !email || !zipcode || city==0 || payment==0 || cost=="none")
	{
