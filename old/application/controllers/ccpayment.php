...
class Ccpayment extends CI_Controller {
	...
	function doku_payment()
	{
		...
		if($cart_list){
			foreach($cart_list as $list2){
				...
				$total2 = $list2['total'];	
				$basket.= trim($name).','.$price.','.$quantity.','.$total2.';';
				$check_total+=$total2;
			}
		...
		if($discount_cart){
			if($check_total>=$discount_cart['minimum_purchase']){
				$discount_price = $discount_cart['discount'];
				$before_discount = $before_discount+$discount_price;
				$basket.='Discount Promo,'.-$discount_price.','.'1'.','.-$discount_price.';';
			}
			else
			{
				$discount_price=0;
		...
		//$basket = "Stars & Stripes Sleveeless Set (0/3 mo),104500,1,104500;Stars & Stripes Casual Set (6/9 mo),104500,1,104500;Kids Rock! Sleeveless Hoodie (5/6 Yr),89500,1,89500;Kids Rock! Stylish Top (3/4 Yr),99500,1,99500;Shipping Cost,15000,1,15000";
		
		//$basket = "Kids Rock Stylish Top (3/4 Yr),99500,1,99500;Shipping Cost,8000,1,8000";
	...
	function doku_payment_guest()
	{
		...
		//$basket = "Item1,199000,2,398000;Item2,123000,1,123000";
		...
		if(check_session_id($session_id)==0)
		{
			$database = array(	'transidmerchant'=>$transaction_id, 
								...
								'shipping_method_fee'=>$shipping_method_fee,
								'voucher_id'=>$voucher_id,
								'subtotal'=>$before_discount, //before_discount
								'discount_price'=>$discount_price,
	...
	function do_checkout()
	{
		...
		if(!$city_detail['jne_province_id'])
			...
			$province_name=api_province_name($city_detail['jne_province_id']);
		...
		if($cart_list)foreach($cart_list as $list){
			if($list['sale_id']==0){
				$price=$list['price'];
				$discount_price=$list['msrp_price']-$price;
				$database = array(	'order_id'=>$order_id,
									'product_id'=>$list['product_id'], 
									'sku_id'=>$list['sku_id'],
									'user_id'=>$user_id,
				...
				$database = array(	'order_id'=>$order_id,
									...
									'quantity'=>$list['quantity'],
									'discount_price'=>$discount_price,
									'price'=>$list['msrp_price'],
									'total'=>$price*$list['quantity']);		
				$this->shopping_cart_model->insert_data('order_item_tb',$database);	
			}
			
			if($list['sale_id']!=0){
				$this->product_model->cut_sku_stock($list['sku_id'], $list['quantity']);
			}
		}
		...
		$this->email->subject('Order Received - '.$number);
