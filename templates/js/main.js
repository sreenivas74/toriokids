	...
	if($("#sku_id_d").val()>0){
		$.ajax({
				...
				success: function(temp){
					...
					ga('send', 'event', 'Shopping', 'Added to Cart', productName);
					}
				}
		});	
...
$(document).ready(function(){
	if($(window).width() >= 640){
		...
		if($('.bannermobileSlider li').length > 1){
			$('.bannermobileSlider').bxSlider({
				auto: true,
				autoHover: true
			});
		};
		...
		$(window).bind('scroll', function(){
			if($(window).scrollTop() > thresholdHeight-300){
				...
				$('#homeHeader').css({
					...
					'background':'#fff url("../userdata/img_headerBG_christmas.png") no-repeat scroll center center',
	...
	itempanelWidth = ($('.itemBox .image').width());
	$('.itemBox .image').css({height:itempanelWidth+'px'});
	
	storeHeight = ($('.storeImage img').width()*0.667);
	$('.storeImage').css({height:storeHeight+'px'});
...
$(window).load(function(){
	...
	bannerHeight = parseInt($('.homeBanner img').height());
	bannermobileHeight = parseInt($('.homemobileBanner img').height());
