...
if($recap)foreach($recap as $list){?>
    ...
    <td valign="top"><?php if($list['payment_type']==1) echo "Bank Transfer"; else echo "Credit Card<br>Transaction ID:[".$list['transidmerchant'].']'?></td>
                ...
                <td valign="top"><?php echo money($list['total']); //echo money($list['total']-$list['discount_price']+$list['shipping_cost']);?></td> 
                ...
                <?php $total+=$list['total']; //$total += ($list['total']-$list['discount_price']+$list['shipping_cost']); $no++; 
	$no++;
	}?>
    </tbody>
</table>
