...
<div id="submenu">
    <ul>
        <li><a class="defBtn" href="<?php echo site_url('torioadmin/order');?>"><span>Back</span></a><?php /*?> <?php if($detail['status']==0 and $detail['payment_type']==1){?><a class="defBtn" href="<?php echo site_url('torioadmin/order/send_reminder').'/'.$detail['id'];?>" onclick="return confirm('Send reminder?');"><span>Send Reminder</span></a><?php }?><?php */?>
        </li>
    </ul>
</div>
<table class="defAdminTable" width="100%">
	<tbody>
        ...
        <tr>
        	<td>Order Date</td>
            <td><?php echo date("d F Y, H:i",strtotime($detail['transaction_date']));?></td>
        </tr>
        <tr>
        	<td>Order Status</td>
            <td>
...
<input type="hidden" name="grand_total" value="<?php echo $detail['total']-$detail['discount_price']+$detail['shipping_cost'];?>" />
                	<select name="status_id" onchange="view_form_jne(this.value)">
                    ...
                    <label id="no_resi" <?php if($detail['no_resi']==''){?>style="display:none"<?php } ?>>  
                   	No Resi
                    <input type="text" class="txtField" name="no_resi" value="<?php echo $detail['no_resi']?>" />
                    </label>
        ...
        <tr>
        	<td>City</td>
            <td><?PHP echo $detail['city_name']?>, <?PHP echo $detail['province_name']?></td>
        </tr>
        <?php /*?><tr>
        	<td>Shipping Cost</td>
    ...
    <input type="submit" class="defBtn" value="Submit"/>
        ...
        <tr>
        	<td>Discount <?php if($detail['total_diskon']>0)
			{
			$discount=(($detail['discount_price']/$detail['total_diskon'])*100); 
			}else
			{
			$discount=(($detail['discount_price']/$detail['total'])*100);
			}echo $discount."%";?></td>
        ...
        <tr>
        	<td>Shipping Cost</td>
            <?php $reg=find('regular_fee',$detail['city'],'jne_city_tb'); 
			$exp=find('express_fee',$detail['city'],'jne_city_tb');
			
			$method=0; if($reg==$detail['shipping_method_fee']) $method=0; if($exp==$detail['shipping_method_fee']) $method=1;  ?>
            <td><?php echo money($detail['shipping_cost']);?> <b>(<?php if($method==0) echo "REG"; if($method==1) echo "EXPRESS"; ?>)</b></td>
        </tr>
        <tr>
        	<td>Grand Total</td>
...
<script>
function view_form_jne(id){
	var no_resi='<?php echo $detail['no_resi']?>';
	if(id==4){
		$("#no_resi").show();
	}else{
		if(no_resi==''){
