...
class Footer_menu_model extends CI_Model{
	...
	{
		$q = "select * from footer_menu_tb order by `precedence`";
	...
	function insert_footer_menu($name, $type, $content_alias, $link, $active, $precedence)
	{
		$q = "insert into footer_menu_tb(`name`, `type`, `content_alias`, `link`, `active`, `precedence`) 
			  values('".esc($name)."',
					 '".esc($type)."',
			  		 '".esc($content_alias)."',
			  		 '".esc($link)."',
	...
	{
		$from=mysql_fetch_assoc(mysql_query('select id, category, precedence from footer_menu_tb where id = '.$id));
		$to=mysql_fetch_assoc(mysql_query('select id, category, precedence from footer_menu_tb where  precedence < '.$from['precedence'].' order by precedence desc'));
	...
	{
		$from=mysql_fetch_assoc(mysql_query('select id, category, precedence from footer_menu_tb where id = '.$id));
		$to=mysql_fetch_assoc(mysql_query('select id, category, precedence from footer_menu_tb where precedence > '.$from['precedence'].' order by precedence asc'));
	...
	function delete_footer_menu($id){
		$q = "delete from footer_menu_tb where id='".esc($id)."'";
		$this->db->query($q);
	}
}