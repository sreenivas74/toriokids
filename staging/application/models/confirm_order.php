...
<div class="<?php echo alternator('midShopCartInfo', 'midShopCartInfo2');?>">
    ...
    <div class="shopCartDetail">
        ...
        <div class="rightBox"><?php if(($list['total']-($list['quantity']*$list['sell_price']))==0)echo "-";else echo "(".money($list['total']-($list['quantity']*$list['sell_price'])).")";?></div>
        <div class="rightBox"><?php echo money($list['quantity']*$list['sell_price']);?></div>
...
<div class="rightBox">-</div>
<div class="rightBox"> <a href="<?php echo site_url('Shopping_cart/deletevoucher/')?>" onclick="return confirm('Confirm Delete?');" >Delete Voucher</a></div>
    </div>
<?php }?>
    ...
    <div class="botRightCartCon">
        <div class="botRightCart">
            <table>
                ...
                <?php if($disc){?>
                <tr>
                    <td width="200"><h3><span style="color:#9E005D;">Discount 20%</span></h3><br />
					<span style="font-size:12px; color:#9E005D;">Soft opening promotion<br />valid until 30 September 2013</span>
					</td>
                    <td width="50">IDR.</td>
                    <td width="150"><div class="totalPrice"><span style="color:#9E005D;"><?php $discount=($total_price*$disc['percentage'])/100; echo money2($discount);?></span></div></td>
                </tr>
                <?php } ?>
                <?php if ($voucher_list) foreach($voucher_list as $list5){?>
                <tr>
                    <td width="200"><span style="color:#9E005D;">Discount Voucher</span><br />
                ...
                <tr>
                    ...
                    <td width="50">IDR.</td>
                    <td width="150" style="text-align:right;"><div class="totalPrice"><?php echo money2($total_price + $shipping_cost - $discount);?></div></td>
                </tr>
         <?php if($user['email']=='cozmo_mb@yahoo.com'){?>
            <?php if($voucher_list==false){?>
            <tr>
        <td widtth="200">Coupoun</td>
        <td width="200"> <form id="voucher_form" name="voucher_form" method="post" action="checkout_summary"> 
        <input class="validate[ajax[check_existing_voucher]]" type="text" name="coupoun" id="coupoun"/><input type="button" class="defBtn" value="Submit" id="submit_btn"> </form></td>
        	<?php } ?>
          </tr>
          <?php ?>
          <tr>
