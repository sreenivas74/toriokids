...
class Product extends CI_Controller{
	...
	{	
		...
		$this->product_model->insert_product($category_name, str_replace('\\','',$sub_category_name), $name, $price, $description, $active, $flag, $weight,$sale_product,$msrp, $best_seller,$featured,$template_id,$discount,$sku);
		$id = mysql_insert_id();
	...
	function do_edit_product($id)
	{
		...
		$this->product_model->update_product($id, $category_name,str_replace('\\','',$sub_category_name), $name, $price, $description, $flag, $weight,$sale_product,$msrp, $best_seller,$featured,$template_id,$discount,$sku);
		$alias = make_alias($name)."_".$id;
		$this->product_model->update_alias($id, $alias);
		redirect('torioadmin/product');
	}
