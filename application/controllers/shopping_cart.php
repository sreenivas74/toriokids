...
class Shopping_cart extends CI_Controller {
	...
	function index(){
		...
		if($this->session->userdata('user_logged_in')==true){
			...
			$this->data['voucher_list']=$data10000=$this->shopping_cart_model->get_discount_guest($session_id);	
		}
					...
					if(!empty($coupoun) && $data['code_voucher']==$coupoun  ){
						...
						if($user_id){
							...
							if($check_coupon_udahdipakai==NULL){
								...
								$this->data['voucher_list']= $this->shopping_cart_model->get_discount_user($user_id);
								$this->data['get_discount_stamps']=$this->coupoun_model->stamps_list($user_id);					
								$discount_content=$this->load->view('content/grand_total_table',$this->data,true);
								//////////////////////////
...
//		if(!$_POST)
	...
	function add_to_cart()
	{
		...
		if(!is_numeric($product_id)){
			echo json_encode(array('status'=>'1','message'=>'error'));exit;
		}
		$real_product  = $this->product_model->get_product_by_id($product_id);
		$check_sku  = $this->product_model->check_product_sku($product_id,$sku_id);

		if(!$real_product){
			echo json_encode(array('status'=>'2','message'=>'404'));exit;
		}
		else{
			$discount_flash_sale = 0;
            if($sale_id!=0){
                $percentage = find('percentage',$sale_id,'flash_sale_tb');
                $discount_flash_sale = $real_product['msrp']-($real_product['msrp']*$percentage/100);
            }
			...
			if($sale_id==0){
				if($schedule_core){
					...
					$real_price = $real_product['msrp'];
				}
			}
			else{
				$real_price = $discount_flash_sale;
			}
			$real_msrp = $real_product['msrp'];
			$aw=$real_product['weight'];
			$actual_weight=$aw*$qty;
			$ceil_weight = ceil($actual_weight);
		}

		if(!$check_sku){
			echo json_encode(array('status'=>'3','message'=>'sku salah'));exit;
		}
		if($sale_id==1){
			if(!$stock){
				echo json_encode(array('status'=>'4','message'=>'stock habis'));exit;
			}
		}
		//if guest
		if($this->session->userdata('user_logged_in')==false){
			$session_id=session_id();
			$cek=$this->shopping_cart_model->check_shopping_cart_guest($product_id,$sku_id,$session_id,$real_price);
			if($cek){
				...
				echo json_encode(array('status'=>'5','message'=>'same'));exit;
			//	redirect($_SERVER['HTTP_REFERER']);
			}
			else{
			...
			$sku_size = find('size', $sku_id, 'sku_tb');
			...
			echo json_encode(array('status'=>'6','qty'=>$qty,'price'=>$price, 'stock'=>$sku_stock, 'cart_qty'=>$cart_qty, 'size'=>$sku_size,'cart_id'=>$cart_id,'product_price'=>$real_price));
		}else{
			//user
			$user_id=$this->session->userdata('user_id');
			$cek=$this->shopping_cart_model->check_shopping_cart_user($product_id,$sku_id,$user_id,$real_price);
			if($cek){
				//update shopping cart
				$actual_weight+=$cek['actual_weight'];
				$ceil_weight = ceil($actual_weight);
	...
	{	
		if($id){
			...
			$province_name = find('name',$list_address['province'],'jne_province_tb');
			$city_name = find('name',$list_address['city'],'jne_city_tb');
			...
			echo json_encode(array('id'=>$list_address['id'],'recipient_name'=>$list_address['recipient_name'],'email'=>$list_address['email'],'shipping_address'=>$list_address['shipping_address'],'phone'=>$list_address['phone'],'province'=>$list_address['province'],'city'=>$list_address['city'],'zipcode'=>$list_address['zipcode'],'province_name'=>$province_name,'city_name'=>$city_name,'jne_kecamatan_name'=>api_city_name($list_address['city'])));
		}
	}
	...
	{	
		...
		//new api
		$this->data['sm'] = $shipping = api_get_city($id);
		//$this->load->view('content/shipping_method_list',$this->data);
		echo json_encode(array('shipping'=>$shipping));
	}

	function check_shipping(){
		$id = $this->input->post('fee');
		$shipping = api_get_city($id);
		pre($shipping);
	}
	...
	function checkout_finish()
	{
		...
		$this->data['content'] = 'content/shopping_thank_you';
		$this->load->view('common/body', $this->data);	
	}
	...
	function delete_shopping_cart(){
		$cart_id = $this->input->post('id');
		$total = find('total',$cart_id,'shopping_cart_tb');
		$this->shopping_cart_model->delete_shopping_cart($cart_id);
		echo json_encode(array('status'=>'produk dihapus','cart_id'=>$cart_id,'total'=>$total));
	}
	...
	function check_stock(){
		...
		if($sale_id!=0){
			if($qty>$stock){
				$qty=$stock;
				echo json_encode(array('status'=>'0','qty'=>$qty));exit;
			}
			else{
				echo json_encode(array('status'=>'1','qty'=>""));exit;
			}
		}
		else{
			echo json_encode(array('status'=>'2','qty'=>""));exit;
		}
	...
	function customer_information(){
		...
		if($this->session->userdata('user_logged_in')){
			...
			$this->coupoun_model->delete_cou_sho_guest($session_id);	
			$cart_list=$this->data['shopping_cart']=$this->shopping_cart_model->get_shopping_cart_list2($session_id);
		...
		//$this->data['city']=$this->jne_model->get_jne_city_data();
		if($this->session->userdata('user_logged_in'))	
		$this->data['content'] = 'content/customer_information';
		else
		$this->data['content'] = 'content/customer_information_guest';
		
		$this->load->view('common/body', $this->data);	
	}

	function do_customer_information(){
		$name = $this->input->post('name');
		$phone = $this->input->post('phone');
		...
		$city_billing = $this->input->post('select_city');
		$api = api_get_city($city_billing);
		$province = $api['jne_province_id'];

		if(!$name || !$name || !$phone || !$address || !$city || !$zipcode){
			redirect('shopping_cart/customer_information');
		}
		else{
			$data = array('customer_guest'=>array('customer_name'=>$name,
							'customer_phone'=>$phone,
							'customer_email'=>$email,
							'customer_address'=>$address,
							'customer_city'=>$city_billing,
							'customer_province'=>$province,
							'customer_zipcode'=>$zipcode,
							'city_billing'=>$city_billing,
							'user_address_id'=>$user_address_id));
			$this->session->set_userdata($data);
			//pre($this->session->userdata('customer_guest'));exit;
			redirect('shopping_cart/shipping_method');
		}
	}
	...
	function shipping_method(){
		...
		$customer_guest=$this->session->userdata('customer_guest');
		if($customer_guest['city_billing']){
			$this->data['sm'] = $shipping = api_get_city($customer_guest['city_billing']);
		}
		//pre($sm);exit;
		...
		$session_id=session_id();
		if($this->session->userdata('user_logged_in')){
			$user_address_id=$customer_guest['user_address_id'];
			// $this->data['shipping_fee']=$_SESSION['shipping_method'];
			// $this->data['shipping_cost']="";
			$this->data['discount']=0;
			// $this->data['total_actual_weight']="";
		...
		$this->data['content'] = 'content/shipping_method';
		$this->load->view('common/body', $this->data);	
	}
	...
	function payment_method(){
		...
		if(!$cart_list){
			...
			$this->session->set_userdata( array('shipping_method'=>$this->input->post('method')) );
			//echo $this->input->post('method');exit;
			$customer_guest=$this->session->userdata('customer_guest');
			$user_id=$this->session->userdata('user_id');
			$this->data['shipping_fee']=$this->session->userdata('shipping_method');
			//echo $this->data['shipping_fee'];exit;
			$session_id=session_id();
		if($this->session->userdata('user_logged_in')){
			...
			$this->data['content']='content/payment_method';
			$this->data['cart_list']=$this->shopping_cart_model->get_shopping_cart_list_user2($user_id);
		}
		else{
			//pre($this->session->userdata);
        	...
        	$this->data['shipping_info']=$address_guest=$this->session->userdata('customer_guest');
			$this->data['discount']=0;
			$this->data['total_actual_weight']="";
			$this->data['disc']=$this->discount_model->get_discount_list();
			
			$check_email=$this->user_model->is_not_guest($address_guest['customer_email']);
			//pre($check_email);
	...
	function payment_summary(){
		...
		if($this->session->userdata('user_logged_in')){
			...
			$this->data['content']='content/payment_summary';
			$this->data['cart_list']=$this->shopping_cart_model->get_shopping_cart_list_user2($user_id);
		}
		else{
			//pre($this->session->userdata);
	...
	function do_checkout_3(){
		...
		if($status==1){
			if(!$this->session->userdata('user_logged_in')){
				...
				///save data user
				$shipping_info=$this->session->userdata('customer_guest');
				////
				$email=$shipping_info['customer_email'];
				
				$full_name=$shipping_info['customer_name'];
				$address=$shipping_info['customer_address'];
				$billing_phone=$shipping_info['customer_phone'];
				$personal_city = $shipping_info['city_billing'];
				$api = api_get_city($personal_city);
				$personal_province = $api['jne_province_id'];
				...
				//$shipping_method_fee=$shipping_info['shipping_cost'];
				$shipping_method_fee=$this->session->userdata('shipping_method');
				//echo pre($shipping_method_fee);exit;
				$session_id=session_id();
				$payment_type=1;
				...
				if(!$check_email){
					...
					$shipping_fee=$this->session->userdata('shipping_method');
					
					$recipient_name=$shipping_info['customer_name'];
					$shipping_address=$shipping_info['customer_address'];
					$phone=$shipping_info['customer_phone'];
					$mobile='';
					
					$city=$shipping_info['city_billing'];
					...
					$zipcode=$shipping_info['customer_zipcode'];
				...
				$shipping_info=$this->session->userdata('customer_guest');
				$user_address_id=$shipping_info['user_address_id'];
