...
class Home_banner extends CI_Controller{
	...
	{	
		...
		$config2['upload_path'] = 'userdata/home_banner';
	...
	function do_edit($id)
	{
		...
		$image_mobile = $old_image['image_mobile'];
		$old = "userdata/home_banner/".$image;
		$old_mobile = "userdata/home_banner/".$image_mobile;
		...
		if($this->upload->do_upload('image_mobile'))
		{	
			if(file_exists($old_mobile))unlink($old_mobile);
			$data = $this->upload->data(); 			
			$image_mobile=$data['file_name'];	
