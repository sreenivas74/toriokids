...
if ( ! function_exists('api_get_city'))
{
	function api_get_city($city_id)
	{
		$parameters = array('city_id'=>$city_id);
		$parameters_json = json_encode($parameters);
		
		$ch = curl_init();  
		$url = "http://jne.isysedge.com/api/jne/get_city_by_id";
...
if ( ! function_exists('api_city_name'))
{
	function api_city_name($city_id)
	{
		$parameters = array('city_id'=>$city_id);
		$parameters_json = json_encode($parameters);
		
		$ch = curl_init();  
		$url = "http://jne.isysedge.com/api/jne/get_city_name";
		curl_setopt($ch,CURLOPT_URL,$url);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
		curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
		curl_setopt($ch, CURLOPT_POSTFIELDS, array("data" => $parameters_json));    
...
if ( ! function_exists('api_province_name'))
{
	function api_province_name($province_id)
	{
		$parameters = array('province_id'=>$province_id);
		$parameters_json = json_encode($parameters);
		
		$ch = curl_init();  
		$url = "http://jne.isysedge.com/api/jne/get_province_name";
		curl_setopt($ch,CURLOPT_URL,$url);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
		curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
		curl_setopt($ch, CURLOPT_POSTFIELDS, array("data" => $parameters_json));    
...
if ( ! function_exists('find_city_name'))
{
	function find_city_name($city_id)
	{
		$parameters = array('city_id'=>$city_id);
		$parameters_json = json_encode($parameters);
...
if ( ! function_exists('find_province_name'))
{
	function find_province_name($province_id)
	{
		$parameters = array('province_id'=>$province_id);
		$parameters_json = json_encode($parameters);
...
if ( ! function_exists('check_coupon_email'))
{
	function check_coupon_email($user_id,$coupon_id)
	{
		//echo 'select '.$coloumn.' from '.$table.' where id='.$id;
		$result=mysql_fetch_assoc(mysql_query("select id from coupon_history_tb where user_id='".$user_id."' AND coupon_id='".$coupon_id."'"));
		return $result['id'];
	}	
}
...
if ( ! function_exists('find_sale'))
{
	function find_sale($coloumn)
	{
		//echo 'select '.$coloumn.' from '.$table.' where id='.$id;
		$result=mysql_fetch_assoc(mysql_query('select a.*, b.percentage as persen from flash_sale_item_tb a join flash_sale_tb b on a.flash_sale_id = b.id where a.product_id = '.$coloumn.' and b.start_time <= "'.date('Y-m-d H:i:s').'" and b.end_time >= "'.date('Y-m-d H:i:s').'" '));
		return $result['persen'];
	}	
}
...
if ( ! function_exists('find_first_image'))
{
	function find_first_image($product_id)
	{
		$sql=" select pi.image from product_image_tb pi join product_tb p on p.id=pi.product_id where p.id='".esc($product_id)."' order by pi.precedence asc limit 1 ";
		$result=mysql_fetch_assoc(mysql_query($sql));
		return $result['image'];
	}	
} 

if ( ! function_exists('check_email_sent'))
{
	function check_email_sent($order_id)
	{
		$sql="select count(id) as count from email_counter_tb where order_id='".esc($order_id)."'";
		$result=mysql_fetch_assoc(mysql_query($sql));
		return $result['count'];
	}	
} 
