...
<div id="content">
    ...
    <dl>
    	...
    	<dl>
            ...
            <dt>
				...
				/*if($category) foreach ($category as $list){?>
                	<p><label>
                        <input id="inlineCheckbox<?php echo $list['id'];?>" type="checkbox"  value="<?php echo $list['id'];?>" name="category_name[]" onclick="get_category_downlink(<?php echo $list['id'];?>)" <?php if(in_array($list['id'],$category_name))echo "checked=\"checked\"";?>/> <?php echo $list['name'];?>
                	</label></p>
					<?php 
					if($sub_category) foreach ($sub_category as $slist){
						if($slist['category_id']==$list['id']){?>
                	<label class="downlink<?php echo $list['id'];?>" <?php if(in_array($list['id'],$category_name))echo 'style="padding-left:15px;"';else echo 'style="display:none;padding-left:15px;"';?>>
                        <input type="checkbox" value="<?php echo $slist['id'];?>" name="sub_category_name[]" <?php if(in_array($slist['id'],$sub_category_name))echo "checked=\"checked\"";?>/> <?php echo $slist['name'];?>
                	</label>
            	<?php 
						}
					}
				...
				if($category) foreach ($category as $list){?>
					...
					<?php $check_category = 0;
					if(in_array($list['id'],$category_name)) $check_category=1;
					if($sub_category) foreach ($sub_category as $slist){
						if($slist['category_id']==$list['id']){
					?>
                	<label class="downlink<?php echo $list['id'];?>" style="display:<?php if($check_category==0) echo "none"; else echo "block"; ?>;padding-left:15px;">
                        <input type="checkbox" value="<?php echo $slist['id'];?>" name="sub_category_name[]" <?php if(in_array($slist['id'],$sub_category_name))echo "checked=\"checked\"";?>/> <?php echo $slist['name'];?>
                	</label>
            	<?php 
						}
        ...
        <dl>
            <dd>SKU</dd>
            <dt><input class="txtField" type="text" name="sku" value="<?php echo $detail['sku_code']?>"/></dt>
        </dl>
        <dl>
            <dd>Weight (kg)</dd>
