...
class Coupoun_model extends CI_Model{
	...
	function delete_cou_sho_guest($session_id){
		$sql = "delete from shopping_cart_tb
											where type=2 AND session_id = '".esc($session_id)."'";											
		$this->db->query($sql);
	}
	...
	function check_active_coupon(){
		$q = "select * from coupon_active_tb";
		$query = $this->db->query($q);
		return $query->row_array();	
	}
	
	function insert_active_coupon($active){
		$q = "insert into coupon_active_tb (active) values ('".esc($active)."')";
		$this->db->query($q);
	}
	
	function update_active_coupon($active){
		$q = "update coupon_active_tb set active='".esc($active)."'";
		$this->db->query($q);
	}
}