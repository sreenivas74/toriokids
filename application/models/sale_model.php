...
class Sale_model extends CI_Model{
	...
	function get_item_by_flash_sale_id($id,$sub_category,$new_arrival,$best_seller,$size){
		$sub1="";
		if($sub_category){
		...
		$q="select a.*, b.flash_sale_id, b.product_id from product_tb a join  flash_sale_item_tb b on b.product_id = a.id where a.active=1 and flash_sale_id='".esc($id)."' ".$search." ".$sub1." ".$new." ".$best." ";
		$query = $this->db->query($q);
		return $query->result_array();
	}

	function get_item_by_flash_sale_id1($id){

		$q="select a.*, b.flash_sale_id, b.product_id from product_tb a join  flash_sale_item_tb b on b.product_id = a.id where flash_sale_id='".esc($id)."' ";
		$query = $this->db->query($q);
		return $query->result_array();
	}
	...
	function get_upcoming_sale($time){
		$q = "select * from flash_sale_tb where start_time>'".esc($time)."' and end_time>'".esc($time)."' limit 1";
		$query = $this->db->query($q);
		return $query->row_array();	
	}
}